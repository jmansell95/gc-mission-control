      ...
      <div className="hub-glass rounded-2xl p-3 sm:p-4">
        <div className="flex flex-col lg:flex-row lg:items-center gap-2.5">
          ...
          <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1 flex-shrink-0">
            {['week', 'month'].map(mode => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition capitalize ${viewMode === mode ? 'bg-white text-[#2E5A1A] shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                {mode}
              </button>
            ))}
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            <button onClick={() => setCurrentDate(new Date())} className="px-2.5 py-2 rounded-lg bg-[#2E5A1A]/10 text-[#2E5A1A] text-xs font-bold hover:bg-[#2E5A1A]/20 transition">
              Today
            </button>
          </div>
          ...
          <div className="relative flex-1 min-w-0">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-200 bg-white text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2E5A1A]/30"
            />
          </div>
          ...
          <div className="flex bg-slate-50 border-b border-slate-200">
            ...
            <div className="flex overflow-x-auto no-scrollbar">
              {days.map(d => (
                <div
                  key={d.dateStr}
                  className={`flex flex-col items-center justify-center min-w-[44px] sm:min-w-[52px] py-1.5 border-r border-slate-100 ${d.isToday ? 'bg-[#2E5A1A]/10' : ''} ${d.isWeekend ? 'bg-slate-100/50' : ''}`}
                >
                  <span className={`text-[9px] font-bold uppercase ${d.isToday ? 'text-[#2E5A1A]' : 'text-slate-400'}`}>{d.dayName}</span>
                  <span className={`text-[11px] font-bold tabular-nums ${d.isToday ? 'text-[#2E5A1A]' : 'text-slate-600'}`}>{d.dayNum}</span>
                </div>
              ))}
            </div>
          </div>
                    ...
                    {days.map(d => {
                      ...
                      return (
                        <button
                          ...
                          className={`relative min-w-[44px] sm:min-w-[52px] h-14 flex flex-col items-center justify-center border-r border-slate-100 transition group ${cfg.bg} ${isToday ? 'ring-1 ring-[#2E5A1A] ring-inset' : ''} ${d.isWeekend ? 'opacity-70' : ''} hover:brightness-110`}
                          title={`${rig.name} · ${d.dateStr} · ${cfg.label}${status?.job_name ? ` · ${status.job_name}` : ''}`}
                        >
                          {isAvailable ? (
                            <>
                              {/* Pulsing gap indicator */}
                              <span className="absolute inset-1 rounded-lg border-2 border-dashed border-slate-300 group-hover:border-[#2E5A1A] transition" />
                              <Plus className="w-3 h-3 text-slate-300 group-hover:text-[#2E5A1A] transition" />
                            </>
                          ) : (
                            <>
