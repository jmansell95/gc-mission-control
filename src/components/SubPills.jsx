      ...
      {pills.map(p => {
        ...
        return (
          <button
            ...
            type="button"
            className={`group relative inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-ui-body font-semibold transition-all flex-shrink-0 sm:flex-1 justify-center sm:justify-start whitespace-nowrap active:scale-[0.97] ${
              isActive
          ...
          >
            ...
            {p.badge != null && p.badge > 0 && (
              <span className={`ml-0.5 px-1.5 py-0.5 rounded-full text-ui-micro font-bold ${isActive ? 'bg-[#1c4a12]/15 text-[#1c4a12]' : 'bg-rose-100 text-rose-600'}`}>{p.badge}</span>
            )}
            {p.count != null && (
              <span className={`ml-0.5 text-ui-caption font-normal ${isActive ? 'text-[#1c4a12]/70' : 'text-slate-400'}`}>{p.count}</span>
            )}
          </button>
        );
      })}
