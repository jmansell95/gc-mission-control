      ...
      <div className="relative ml-auto h-full w-full sm:max-w-[1200px] bg-white shadow-2xl flex flex-col animate-drawer-slide-in">
        ...
        <div className="hub-glass px-5 py-4 flex-shrink-0 border-b border-slate-200">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#2E5A1A] to-[#5A8C1E] flex items-center justify-center flex-shrink-0 shadow-sm">
                <Truck className="w-6 h-6 text-white" />
              </div>
              <div className="min-w-0">
                <p className="font-mono font-bold text-lg text-slate-900 truncate">{vehicle.registration_number || 'No Reg'}</p>
                <p className="text-sm text-slate-500 truncate">{makeModel || vehicle.name || 'Vehicle'}</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <button onClick={handleDownloadReport} disabled={reportLoading}
                className="p-2 hover:bg-slate-100 rounded-lg transition flex items-center gap-1.5 text-xs font-semibold text-slate-600 disabled:opacity-50">
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {isMobile && (
              <div className="sticky top-0 bg-white/80 backdrop-blur-sm border-b border-slate-200 px-3 py-2 z-10">
