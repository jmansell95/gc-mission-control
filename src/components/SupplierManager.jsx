...
export default function SupplierManager() {
  ...
  const handleAddCategoryInline = async () => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, category: [...(Array.isArray(prev.category) ? prev.category : []), value] }));
      setNewCategoryName('');
  ...
  const startEdit = (s) => {
    setForm({
      name: s.name, category: Array.isArray(s.category) ? s.category : (s.category ? [s.category] : []), contacts: s.contacts || [], notes: s.notes || '',
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="flex items-center gap-1.5 text-sm font-medium text-slate-700 mb-1.5">
                <Tag className="w-3.5 h-3.5 text-slate-400" /> Categories <span className="text-xs text-slate-400 font-normal">(select all that apply)</span>
              </label>
              {!addingCategory ? (
                <div className="flex flex-wrap gap-1.5">
                  {categoryOptions.map(opt => {
                    ...
                    return (
                      <button key={opt.value} type="button" onClick={() => setForm({ ...form, category: selected ? cats.filter(c => c !== opt.value) : [...cats, opt.value] })} className={`px-2.5 py-1 rounded-full text-xs font-bold transition ${selected ? getCategoryPillClass(opt.value) : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
                        {opt.label}
                      </button>
                    );
                  })}
                  <button type="button" onClick={() => setAddingCategory(true)} className="inline-flex items-center gap-0.5 px-2.5 py-1 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold transition">
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {(Array.isArray(s.category) ? s.category : (s.category ? [s.category] : [])).length === 0 ? (
                        <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500">Uncategorised</span>
                      ) : (
                        (Array.isArray(s.category) ? s.category : [s.category]).map(cat => (
                          <span key={cat} className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold ${getCategoryPillClass(cat)}`}>
