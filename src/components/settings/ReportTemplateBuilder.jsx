      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="w-full lg:w-80 flex-shrink-0 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            <button onClick={() => setCatFilter('all')} className={`px-2.5 py-1 rounded-full text-[11px] font-semibold transition ${catFilter === 'all' ? 'bg-primary text-white' : 'bg-white text-slate-500 border border-slate-200'}`}>All</button>
            {Object.entries(CATEGORY_META).map(([k, v]) => (
              <button key={k} onClick={() => setCatFilter(k)} className={`px-2.5 py-1 rounded-full text-[11px] font-semibold transition ${catFilter === k ? 'bg-primary text-white' : 'bg-white text-slate-500 border border-slate-200'}`}>{v.label}</button>
            ))}
          </div>
          {isLoading ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                      ...
                      {Object.entries(CHART_META).map(([k, v]) => {
                        ...
                        return (
                          <button key={k} type="button" onClick={() => setFormData(f => ({ ...f, chart_type: k }))}
                            className={`flex flex-col items-center gap-1 py-2.5 rounded-lg border text-[10px] font-semibold transition ${formData.chart_type === k ? 'border-primary bg-emerald-50 text-primary' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <input type="text" value={fieldInput} onChange={e => setFieldInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addField(); } }} placeholder="Add a field name"
                        className="flex-1 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-primary" />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <select value={formData.schedule_cadence} onChange={e => setFormData(f => ({ ...f, schedule_cadence: e.target.value }))} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-primary bg-white">
                  ...
                  <label className="flex items-center gap-2 text-sm text-slate-700">
                    <input type="checkbox" checked={formData.is_shared} onChange={e => setFormData(f => ({ ...f, is_shared: e.target.checked }))} className="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" />
