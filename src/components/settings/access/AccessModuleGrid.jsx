...
import { hubHasSubTabs, getAllPermissionKeysForHub } from '@/utils/subTabRegistry';
...
export default function AccessModuleGrid({
  permissions, isReadOnly, onChange, onSetAll,
  subTabPermissions = {}, onSubTabChange, onSetHubSubTabs,
}) {
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-start gap-2 p-2.5 rounded-xl bg-slate-50 border border-slate-200">
        ...
        <p className="text-[11px] text-slate-600 font-medium">
          <span className="font-bold">Default Deny</span> — when sub-tab permissions are configured, any sub-tab not explicitly granted is hidden. New tabs added in future default to <span className="font-bold">No Access</span> until an admin grants them. Click the <ChevronRight className="w-3 h-3 inline -mt-0.5" /> arrow on a hub to configure individual tabs.
        </p>
      </div>
    </div>
  );
