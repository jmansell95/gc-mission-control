      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-5 max-h-[calc(100dvh-2rem)] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        {selectedCourses.length > 0 && (
          <div className="mb-4 p-3 bg-primary/5 rounded-xl border border-primary/15">
            <p className="text-[10px] font-bold text-primary uppercase tracking-wide mb-2">Selected Courses ({selectedCourses.length})</p>
            <div className="flex flex-wrap gap-1.5 mb-3">
              {selectedCourses.map(c => (
                <div key={c.id} className="inline-flex items-center gap-1.5 pl-2.5 pr-1 py-1 bg-white border border-primary/20 rounded-full text-xs font-medium text-slate-700 shadow-sm">
                ...
                return (
                  <div key={c.id} className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-100">
                    ...
                    <select value={currentProviderId} onChange={e => setProviderOverrides(prev => ({ ...prev, [c.id]: e.target.value }))}
                      className="px-2 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:border-primary bg-white max-w-[160px]">
                ...
                return (
                  <button key={c.id} onClick={() => toggleCourse(c.id)}
                    className={'w-full flex items-center gap-3 p-3 rounded-xl border transition text-left ' +
                      (isSelected ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-slate-200 bg-white hover:bg-slate-50')}>
                    <div className={'w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ' + (isSelected ? 'bg-primary' : 'bg-blue-100')}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {(c.provider_id || c.provider) && <p className="text-[10px] text-primary font-medium flex items-center gap-0.5 mt-0.5"><Building2 className="w-2.5 h-2.5" />{providerName(c.provider_id) || c.provider}</p>}
                    </div>
                    {isSelected && (
                      <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full flex-shrink-0">Added</span>
                    )}
                  </button>
                );
              })}
        ...
        <div className="mb-4">
          ...
          <div className="relative mb-2">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search crew…"
              className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10" />
          </div>
            ...
            {filteredStaff.map(s => {
              ...
              return (
                <button key={s.id} onClick={() => toggleStaff(s.id)}
                  ...
                  <div className={'w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ' + (checked ? 'bg-primary border-primary' : 'border-slate-300 bg-white')}>
