      ...
      <div className="bg-white w-full max-w-4xl max-h-[calc(100dvh-2rem)] rounded-2xl shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 border-b border-slate-100">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10"
            />
          </div>
          <div className="flex items-center gap-4 mt-2.5 text-[11px] text-slate-500">
            <span className="flex items-center gap-1.5"><Pin className="w-3 h-3 text-primary" /> Pinned = always visible</span>
        ...
        <div className="px-5 py-3 border-b border-slate-100 bg-emerald-50/30">
          <p className="text-xs font-bold text-slate-700 mb-1.5 flex items-center gap-1.5">
            <Pin className="w-3.5 h-3.5 text-primary" /> Pinned GC Template Codes
          </p>
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {Array.from(codes).map(c => (
              <span key={c} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary text-white text-xs font-bold">
                {c}
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary uppercase"
            />
            <button onClick={addCode} className="px-3 py-1.5 bg-primary text-white rounded-lg text-sm font-bold hover:bg-[#244715] transition">Add Code</button>
          </div>
        </div>
                ...
                >
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition ${
                      t.isPinned ? 'bg-primary text-white' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'
                    }`}
        ...
        <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="inline-flex items-center gap-1.5 px-5 py-2 bg-primary text-white rounded-xl text-sm font-bold hover:bg-[#244715] transition disabled:opacity-50"
            >
