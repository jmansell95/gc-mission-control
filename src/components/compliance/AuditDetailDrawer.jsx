...
function CheckSection({ section, items, defaultOpen = true, onCreateIncident }) {
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.status === 'fail' && (
                    <button
                      ...
                      className="mt-1.5 inline-flex items-center gap-1 text-[10px] font-bold text-rose-600 hover:text-rose-800 transition"
                    >
...
export default function AuditDetailDrawer({ audit, onClose }) {
  ...
  const [showIncidentModal, setShowIncidentModal] = useState(null); // null | 'whole' | item object
  ...
  // Aggregate ALL photos from all check items for the gallery strip
  ...
  const handleSaveActions = async () => {
    ...
    try {
      const updated = actionItems.map((a, i) => ({
        ...a,
        ...(actionDraft[i] || {}),
      }));
      ...
      <div className="fixed inset-0 z-[60] bg-slate-950/60 backdrop-blur-md flex justify-end" onClick={onClose}>
        ...
        >
            ...
            <div className="px-5 pb-3 flex items-center gap-2">
              {reportUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center justify-center gap-2 flex-1 px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 transition shadow-sm"
                >
              ...
              {(failed || itemsFailed > 0) && (
                <button
                  onClick={() => setShowIncidentModal('whole')}
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600 text-white text-sm font-bold hover:bg-rose-700 transition shadow-sm"
                >
          ...
          <div className="p-5 space-y-4">
            ...
            {/* Photo gallery strip — all photos from the audit */}
            {!detailLoading && allPhotos.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <ImageIcon className="w-4 h-4 text-[#2E5A1A]" />
                  <h4 className="text-sm font-bold text-slate-900">Audit Photos ({allPhotos.length})</h4>
                </div>
                <PhotoGalleryLightbox photos={allPhotos} label="All audit photos" />
              </div>
            )}
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {itemsBySection.map(([section, items]) => (
                        <CheckSection key={section} section={section} items={items} onCreateIncident={(item) => setShowIncidentModal(item)} />
                      ))}
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {Object.keys(actionDraft).length > 0 && (
                      <button
                        ...
                        className="ml-auto inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#2E5A1A] text-white text-xs font-bold hover:bg-[#1c4a12] transition disabled:opacity-50"
                      >
                          ...
                          <div key={i} className="p-3 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              ...
                              <input
                                ...
                                onChange={(e) => updateActionField(i, 'assignee', e.target.value)}
                                placeholder="Assignee"
                                className="px-2 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:border-[#2E5A1A] bg-white"
                              />
                              {/* Due date */}
                              <input
                                type="date"
                                value={currentDue}
                                onChange={(e) => updateActionField(i, 'due_date', e.target.value)}
                                className="px-2 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:border-[#2E5A1A] bg-white"
                              />
                              {/* Status */}
                              <select
