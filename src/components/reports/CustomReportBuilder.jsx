            ...
            <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto">
              {COMMON_FIELDS.map(f => (
                <button key={f.key} onClick={() => toggleField(f.key)}
                  className={`text-xs px-2.5 py-1.5 rounded-lg font-medium transition ${selectedFields.includes(f.key) ? 'bg-[#2E5A1A] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                  {f.label}
                </button>
              ))}
            </div>
                  ...
                  {CHART_TYPES.map(ct => {
                    ...
                    return (
                      <button key={ct.id} onClick={() => setChartType(ct.id)}
                        className={`p-1.5 rounded-md transition ${chartType === ct.id ? 'bg-white text-[#2E5A1A] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        title={ct.label}>
