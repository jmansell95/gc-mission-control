...
export default function HubErrorState({ title = 'Something went wrong', description, error, onRetry, compact = false }) {
  ...
  return (
    <div className={`hub-glass rounded-3xl text-center animate-slide-up border-rose-100 ${compact ? 'p-5' : 'p-10'}`}>
      ...
      <h3 className={`${compact ? 'text-ui-body' : 'text-ui-subheading'} font-bold text-slate-900`}>{title}</h3>
      {(description || detail) && (
        <p className="text-ui-body text-slate-500 max-w-md mx-auto mt-1">{description || detail}</p>
      )}
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-4 inline-flex items-center gap-1.5 h-9 px-3 rounded-xl bg-slate-900 text-white text-ui-caption font-semibold hover:bg-slate-800 active:scale-[0.98] transition"
        >
