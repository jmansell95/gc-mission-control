...
export default function HubQuickLinks({ hubKey, links }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-nowrap overflow-x-auto no-scrollbar -mx-1 px-1 sm:flex-wrap sm:overflow-visible animate-slide-up">
      <span className="text-ui-micro uppercase tracking-wider text-slate-400 flex items-center gap-1 flex-shrink-0">
      ...
      {related.map(hub => {
        ...
        return (
          <Link
            ...
            to={hub.to}
            state={hub.state}
            className="group inline-flex items-center gap-1.5 h-9 px-3 flex-shrink-0 bg-white/80 backdrop-blur border border-slate-200 hover:border-primary/30 hover:bg-primary/5 rounded-xl text-ui-caption font-semibold text-slate-600 hover:text-primary transition-all duration-200 shadow-sm active:scale-[0.97]"
          >
